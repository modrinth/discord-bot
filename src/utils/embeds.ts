...
function parseColor(input: string | undefined): ColorResolvable | undefined {
  ...
  if (/^0x[0-9a-fA-F]{6}$/.test(v))
    return parseInt(v, 16) as unknown as ColorResolvable
  const n = Number(v)
...
export function createDefaultEmbed(data?: EmbedData): EmbedBuilder {
  ...
  if (authorName)
    builder.setAuthor({ name: authorName, iconURL: authorIconURL || undefined })

  const footerText = process.env.EMBED_FOOTER_TEXT?.trim()
  const footerIconURL = process.env.EMBED_FOOTER_ICON_URL?.trim()
  if (footerText)
    builder.setFooter({ text: footerText, iconURL: footerIconURL || undefined })
